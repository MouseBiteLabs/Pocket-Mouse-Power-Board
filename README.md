...
If you are assembling this yourself, I suggest testing the assembled board externally before mounting it to your Game Boy. This can be done by connecting pins 2 to the positive terminal of an input voltage source with at least 2 V output (using battery clips or an external power supply), letting the capacitors charge up for a bit, then connecting pin 1 to pin 2. Measure the output voltage on pin 7 (remember to connect pin 3 to the ground/negative end of the power source). Pin 7 should read a consistent ~5 V. If this is complicated to test, you can instead add the assembled board to your Game Boy PCB, but keep pin 7 empty. Insert batteries into the back shell, wait a few seconds for the capacitors to charge up, turn the power switch, and measure pin 7 with a meter to confirm you are getting a solid 5 V output, after which you can connect pin 7 to the main board.
